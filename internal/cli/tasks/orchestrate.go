...
type OrchestrateOptions struct {
	...
	// Interactive controls whether the running phase (planner when
	// not SkipPlanning, otherwise worker) runs in TUI mode. Defaults
	// to false (headless). Set by `j tasks start --interactive` and
	// the resume-* CLI wrappers.
	Interactive bool
...
func newOrchestrateCmd() *cobra.Command {
	...
	cmd.Flags().Bool("interactive", false, "Run the active phase (planner without --skip-planning, otherwise worker) in TUI mode")
