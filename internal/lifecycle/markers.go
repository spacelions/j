...
// agentlog.EmitTo.
//
// The helper swallows agentlog write errors. Markers are
// observability signal, not load-bearing data — losing one is
// strictly less harmful than aborting a phase begin.
func emitPhaseBegin(agentLogPath, phase string, t tasks.Task) {
...
// duration_ms is computed from beginAt when non-zero so the marker
// is self-contained; outcome is one of `done` / `help` / `pass` /
// `fail` per the agent.log marker convention. An empty agentLogPath
// is a silent no-op.
func emitPhaseEnd(agentLogPath, phase string, beginAt time.Time, t tasks.Task, outcome string) {
