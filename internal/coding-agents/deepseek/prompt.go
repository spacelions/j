...
// claude backend exactly: a fresh run composes the planner instruction;
// a resume run switches to the resume-only template that asks the
...
// verify loop wants the previous worker session to address a concrete
// set of verifier findings without re-planning). Resume runs are next;
// first-run falls through. Every branch threads req.ClarificationPath
// so the per-task escape hatch is preserved.
