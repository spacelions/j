...
// That edge must be a legal transition from
// `needs-clarification` back to `working` so the orchestrator can
// re-run the worker and (on a clean exit) progress to the verifier.
func TestFSM_WorkResumeFromNeedsClarification(t *testing.T) {
	if !tasks.IsLegal(
		tasks.StatusNeedsClarification, tasks.EventWorkResume,
	) {
		t.Fatal("IsLegal(needs-clarification, work_resume) = false," +
			" want true (resume after clarification)")
	}
	got, err := tasks.Apply(
		tasks.StatusNeedsClarification, tasks.EventWorkResume)
	if err != nil {
