...
// TestTasksShowFindings_EmptyStore pins acceptance criterion #3:
// `j tasks show findings` (no `--from-task`) on an empty store
// short-circuits BEFORE the picker, prints `J: no tasks`, and
// exits 0.
func TestTasksShowFindings_EmptyStore(t *testing.T) {
	t.Chdir(t.TempDir())
	testutil.Init(t)
